...
POPPLER_BIN = r"C:\poppler-25.07.0\Library\bin"  # ensure this exists on your machine
...
def embed_text_gemini(text: str) -> np.ndarray:
    """Return normalized embedding (float32, L2-normalized)."""
...
def build_faiss_index(doc_id: int, chunks: list[str]):
    """Create per-document FAISS index + metadata.
       Saves to instance/indexes/{doc_id}.faiss and {doc_id}.meta.json
    """
    if not chunks:
        return

    vecs = [embed_text_gemini(ch) for ch in chunks]
    X = np.vstack(vecs).astype(np.float32)  # normalized
...
def upload():
    if request.method == 'POST':
        ...
        print("index built for doc:", doc.id)

        flash("Document uploaded, extracted & indexed (FAISS).")
        return redirect(url_for('docs_bp.upload'))

    # GET — show latest doc state (no history)
    # GET — show latest doc state (no history)
    ...
    latest_exists = False
    if latest and latest.extracted_text and latest.extracted_text.strip():
        latest_exists = True
...
def summarize_latest():
    ...
    target_words = max(35, int(total_words * 0.35))  # ensure a minimum length
